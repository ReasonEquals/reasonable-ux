...
- **Dedup-pass unit test.** The 7-line dedup loop in `stitch_reports` (`generate_report.py:287`) is inline and untested. Add `tests/test_dedup_findings.py` if the logic grows beyond a few lines, or extract to a helper function first. Considered in Batch 50 and skipped to avoid a refactor-just-to-test.
- **PDF page section breaks.** Merged multi-page PDF has no visual divider between URL groups in the step list — template not yet updated. Low priority until portfolio runs are final.
...
- **Regenerate `DECISIONS.pdf` whenever `DECISIONS.md` changes.** The PDF is tracked (it's the portfolio artifact for non-developer readers — recruiters/founders shouldn't need to install Playwright). Run `python render_decisions.py` after any DECISIONS.md edit and stage both files together. If you commit DECISIONS.md without the PDF, the public artifact silently drifts from the source.
