...
- **Advisor invocation tracking diagnosis.** Six v2/v4 pre-flights run during Batch 71 came back with `advisor_called_count=0` AND `advisor_eligible_steps=0` despite `--advisor` on the CLI. Either the Batch 70 visibility instrumentation is dropping the flag through `run.py`'s multi-page path, or the advisor tool genuinely fires rarely (33% per-step cost premium vs `v1_baseline` is consistent with "tool def in context, tool rarely invoked"). Diagnose via Langfuse trace: does the advisor tool appear in the request, does Sonnet ever emit a tool-use block, does the post-run aggregation lose the count? If (a), fix the tracking and the existing "advisor mispricing" claim becomes more interpretable. If (b), the cost premium isn't even buying advisor calls — it's just longer prompts. See DECISIONS.md §9 entry "2026-04-29 unexpected finding" for the data. Likely batch 72.
