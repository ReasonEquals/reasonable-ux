...
- **Langfuse blindspot runtime verification.** Partially resolved (Batch 44): all 4 LiteLLM step traces confirmed in dashboard after flush delay (initially showed 2, settled at 4). OTel "Proxy Server not installed" warning is benign. Remaining: run with `--advisor` to confirm `_complete_anthropic_advisor` span appears; below-fold and scout paths covered by `AnthropicInstrumentor` but not yet spot-checked as distinct traces.
