...
**`max_tokens` budget in the agent loop.** Current caps: **2048 when advisor is enabled, 1024 otherwise**, at `tests/agent_test.py:842` — `step_budget = 2048 if advisor else 1024`. The 2048 branch was added in batch 17 to accommodate the Opus advisor's `tool_use` response blocks. The JSON schema in `_build_prompt` is deliberately compact to fit inside these budgets with room for a 3–5 friction-point list. **Don't add fields without shrinking existing ones**, and **ask before raising either cap** — cost scales linearly with output tokens and raising silently has bitten past sessions.
