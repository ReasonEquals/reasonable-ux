...
reasonable-ux started as a single Playwright + Claude loop that took a screenshot, asked "what do you see?", and clicked the next thing. By the end it was 117 commits: multi-provider LLM routing, authenticated multi-page crawls, Langfuse observability with a hard-won memory-leak fix, a calibrated eval harness with regression detection, and a PDF reporting pipeline backed by Haiku synthesis. This document records the decisions that shaped the system — what was chosen, what was considered, what was given up, and what broke along the way.

---

## 1. Token Economics

Vision models are expensive per-call. Multi-step runs with 8–12 screenshots, a JSON schema, and a growing conversation history compound fast. Every decision in this section was made under the constraint: keep a 12-step run under a cost target without sacrificing report quality.
...
**Decision:** Before appending the current step's screenshot, walk all prior user messages and drop any `"type": "image"` content blocks. Only the new step's screenshot travels in the payload. [`agent_core.py:849`](agent_core.py#L849)
...
**Outcome:** Approximately 84% token reduction per run. Enabled 12-step defaults without hitting cost ceilings. The claim holds up empirically across run logs.
...
**Outcome:** Approximately 5,000 fewer tokens per run across a 12-step suite. No reported fidelity regressions in UX scoring.
...
**Tradeoffs accepted:** Sonnet emitted malformed JSON in approximately 40% of eval runs initially. This required a `json-repair` fallback (Batch 32) and a full baseline recalibration (Batch 33). The quality gap is real but acceptable for a tool that surfaces directional UX signals, not precise scores.

**Outcome:** Cost down 11× ($8 → $0.71 per eval run). Eval pass rate recovered from 5/20 → 19/20 after recalibration. Haiku runs exec synthesis for approximately $0.002 per report.
...
**Decision:** `step_budget = 2048 if advisor else 1024`. The budget check fires after the call is logged, raising an exception that exits the loop gracefully. [`agent_core.py:116`](agent_core.py#L116)
...
**Decision:** `LLMAdapter.complete()` routes everything through `litellm.acompletion()` ([`agent_core.py:147`](agent_core.py#L147)) except when advisor mode is active, which routes directly to `_complete_anthropic_advisor()` ([`agent_core.py:168`](agent_core.py#L168)). The adapter class unifies tracing across both paths. [`agent_core.py:123`](agent_core.py#L123)
...
**Tradeoffs accepted:** Two code paths to maintain. When `--advisor` is enabled with a non-Anthropic provider, the advisor path silently falls back to the default Anthropic model.
...
**Decision:** `_do_auth()` ([`run.py:124`](run.py#L124)) logs in once via a dedicated Playwright context, calls `context.storage_state()` (cookies + localStorage), writes the result to a tempfile, and passes the tempfile path to every subsequent page evaluation via Playwright's `storage_state=` parameter. [`run.py:180`](run.py#L180)
...
**Tradeoffs accepted:** Slower wall-clock time for large suites. A 5-page run is 5× slower than it needs to be in theory.
...
**Outcome:** All three paths now surface as named generations in Langfuse under the correct session. The fix is stable across Python versions.
...
**Decision:** A compiled regex pattern at [`evals/run_evals.py:137`](evals/run_evals.py#L137) matches CSS anti-patterns (`a[href`, `a:has-text`, `.nav-`, `nav a`) in step `target` fields. Labels with `assert_nav_drift: true` fail the eval if the pattern matches any step. All 7 `saas_landing` labels carry this flag. [`evals/run_evals.py:143`](evals/run_evals.py#L143), [`evals/run_evals.py:194`](evals/run_evals.py#L194)
...
**Decision (Batch 28):** Fixed 6 label bugs, widened score bands by ±12. Result: 65% pass rate. **Decision (Batch 28.5):** Fixed 4 more label bugs, recalibrated bands around Sonnet 4.6's actual scoring behavior after the model swap. Result: 19/20 (95%) before locking the baseline.
